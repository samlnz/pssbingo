...
class GamePage {
    ...
    init() {
        ...
        // Start with READY animation
        this.showReadyAnimation();
    }

    setupServerListeners() {
        // Listen for number calls from server
        this.serverClient.on('number-called', (numberData) => {
            this.handleServerNumberCall(numberData);
        });
        ...
        // Listen for winner
        this.serverClient.on('winner-declared', (winnerData) => {
            this.handleWinner(winnerData);
        });
    }

    handleServerNumberCall(numberData) {
        if (!this.isGameActive) {
            this.startGame();
        }
        
        // Update game state
        this.gameState.calledNumbers.add(numberData.number);
        
        // Update display
        this.updateNumberDisplay(numberData.number, numberData.letter);
    ...
    startGame() {
        ...
        this.startGameTimer();
        BingoUtils.showNotification('Game started!', 'success');
